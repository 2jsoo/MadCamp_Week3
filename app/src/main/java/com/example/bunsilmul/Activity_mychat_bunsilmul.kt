...
class Activity_mychat_bunsilmul: AppCompatActivity() {
...
//        text_information.text = information
        ...
        intent.getStringExtra("id")?.let {
            val call = BunsilmulApiObject.retrofitService.GetBunsilmulbyID(it)
            call.enqueue(object : retrofit2.Callback<bunsilmul> {
                override fun onFailure(call: Call<bunsilmul>, t: Throwable) {
                    Log.d("BunsilmulCreate", "Fail")
                }

                override fun onResponse(
                        call: Call<bunsilmul>,
                        response: retrofit2.Response<bunsilmul>
                ) {
                    if (response.isSuccessful) {
                        response.body()?.let {
                            it.photo?.let{
                                photo.setImageBitmap(stringToImage(it))
                            }
                            it.information?.let{
                                text_category.text = it
                                information = it
                            }
                            it.category?.let{
                                text_information.text = it
                                category = it
                            }
                            it.uid?.let{
                                objectuid = it
...
//            chat_button.isEnabled = true
        ...
        chat_button.setOnClickListener {
            val intent = Intent(this@Activity_mychat_bunsilmul, ChatActivity::class.java)
